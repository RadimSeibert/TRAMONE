...
void sendATCommand(String command, const int timeout, boolean debug, String &gsm_message) {
  ...
  while ((time + timeout) > millis()) {
    ...
    if (gsm_message.length() > 0) {
      if (debug) {
        Serial.print(gsm_message);
      }
      delay(250);
      break;
    }
...
void reconnect_mqtt(long int starttime) {
...
void loop() {
 ...
 if (!modem.isNetworkConnected()) {
  ...
  wdt_reset();
  String gsm_message;
  sendATCommand("AT+CIPSTATUS",1,1,gsm_message);
  if (gsm_message.indexOf("CONNECT OK") == -1) {
        while(true);
  }
...
// *******************************
   ...
   //*****************
// read temperature + heating control
